...
def evaluate_model_link_prediction(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, time_gap: int = 2000, full_return : bool = False, temp : int = 1, return_batch_losses : bool = False):
    """
    ...
    with torch.no_grad():
        ...
        non_existant_links = []
        evaluate_idx_data_loader_tqdm = tqdm(evaluate_idx_data_loader, ncols=120, mininterval=120)
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            if full_return :
                ...
                for src, dst, label, timestamp in zip(batch_src_node_ids, batch_dst_node_ids, batch_labels, batch_node_interact_times):
                    all_actual_links.append((int(src), int(dst), int(label), float(timestamp)))  

                # Store predictions with scores for actual links only
                for src, dst, score, timestamp in zip(batch_src_node_ids, batch_dst_node_ids, positive_probabilities, batch_node_interact_times):
                    all_predicted_links.append((int(src), int(dst), float(score), float(timestamp))) 
                
                for src, dst, score, timestamp in zip(batch_neg_src_node_ids, batch_neg_dst_node_ids, negative_probabilities, batch_node_interact_times):
                    non_existant_links.append((int(src), int(dst), float(score), float(timestamp))) 

    if full_return :
        return evaluate_losses, evaluate_metrics, all_predicted_links, all_actual_links, non_existant_links
